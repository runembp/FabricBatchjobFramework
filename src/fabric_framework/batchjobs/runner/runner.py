class Runner:
    ...
    def run(self, batch_job):
        batchjob_id = batch_job.batchjob_id
        required_columns = batch_job.required_columns
        entity_class = batch_job.entity_class

        self.spark.increment_retry_count_on_failed_records(batchjob_id)
        dataframe_transformed = batch_job.map_csv_file(self, batchjob_id)
        self.spark.prepare_transformed_table(batchjob_id, dataframe_transformed)
        batch_job.add_lookup_and_prepare_ingest(batchjob_id, dataframe_transformed)
